...
* Quickly check and adjust load frames with the load checking window
...
2. You'll be met with the conversion window, here you could cut to the part of the video where the run actually takes place. If the game you're is locked at a lower framerate than the video then you want to change the FPS setting to match that (Example: a game runs at 30fps while the video is recorded at 60). Hit convert when you're happy with your settings.
3. After converting you'll be met with the main window. Find the frame where the speedrun start using the navigation down at the bottom, then hit "Set Start". Do the same for the frame where the speedrun ends (the last frame of gameplay) and hit "Set End".
...
Now you can export your frame count using the "export times" button, however it's recommended that you double check the detected loads using the steps below.
